...
```
Starting a project:  spec-driven-development → design-flow → planning-and-task-breakdown
...
The `/spec`, `/design`, and `/plan` commands create working artifacts under `work/<project-name>/`, such as `work/<project-name>/specs/<feature>.md`, `work/<project-name>/design/<feature>-design-requirements.md`, `work/<project-name>/tasks/<feature>-plan.md`, and `work/<project-name>/tasks/<feature>-todo.md`. Project-specific app/source directories should also live under the same project folder, such as `work/<project-name>/apps/mobile`. Treat these as **living documents** while the work is in progress:
...
2. **Use design-flow before planning user-facing work**
3. **Always load test-driven-development** when writing code
4. **Don't skip verification steps** — they're the whole point
5. **Load skills selectively** — more context isn't always better
