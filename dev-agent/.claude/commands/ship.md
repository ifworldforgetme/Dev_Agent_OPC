...
Write launch notes and release artifacts under `work/<project-name>/ship/`. Use project-local app/source paths such as `work/<project-name>/apps/mobile` when running build, test, and packaging commands.
...
5. **Infrastructure** — Env vars, migrations, host SDKs/permissions via `bin/dev-flow env-check <project-name>`, monitoring, feature flags. Verify directly.
