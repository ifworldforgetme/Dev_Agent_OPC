...
1. Read `reviews/VERIFICATION.md`, current source, and the design handoff when UI applies.
2. For `AUTOMATED_QA`, run functional and exploratory/monkey checks and record `FUNCTIONAL_TEST.md` and `MONKEY_TEST.md`.
3. For `VISUAL_QA`, compare implemented UI with required design contract inputs and record `VISUAL_COMPARISON.md` with `Overall score: N/100`.
